...
void write_vector_in_file(const std::vector<double> &v, std::ofstream &out)
    {
        for (auto item : v)
        {
            out << item << " ";
...
void write_two_vectors(const std::string &path, const std::vector<double> &E_set, const std::vector<double> &N_set)
    {
        std::ofstream out_E(path, std::ios::ate);
        write_vector_in_file(N_set, out_E);
        write_vector_in_file(E_set, out_E);
        out_E.close();
    }

int main()
    {
        std::vector<double> E_set, E_Mer_set, V_set, V_Mer_set, N_set, Cor_set, Cor_Mer_set, Cor2N_set, Cor2N_Mer_set;
        for (int N = 10; N < 10e6; N *= 5)
        {
            N_set.push_back(N);
            {
...
// 1) Выборка:
                ...
                N_str << N;
                std::ofstream out_("../data/Source_data/" + N_str.str() + ".txt");
                std::ofstream out_Mer("../data/Source_data/Mer" + N_str.str() + ".txt");

                write_vector_in_file(sample, out_);
                write_vector_in_file(sample_Mer, out_Mer);
                out_.close();
                out_Mer.close();
